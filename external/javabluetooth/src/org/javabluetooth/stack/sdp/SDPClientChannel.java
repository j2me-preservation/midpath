...
public class SDPClientChannel extends L2CAPChannel {
	...
	public void send_SDP_ServiceSearchRequest(short transactionID, short maxRecordsReturned, DataElement uuidListElement)
			throws IOException {
	...
	public void send_SDP_ServiceAttributeRequest(short transactionID, short maxAttributesReturned,
			int serviceRecordHandle, DataElement attributeListElement) throws IOException {
	...
	private void receive_SDP_ErrorResponse(byte[] dataPacket) {
		...
		case 2:
			...
			break;
		case 3:
			System.err.println("SDP: Received Error Response: Invalid request syntax. (transactionID=" + transactionID
					+ ")");
			break;
		case 4:
		...
		case 6:
			System.err
					.println("SDP: Received Error Response: Insufficient Resources to satisfy Request. (transactionID="
							+ transactionID + ")");
			break;
		default:
		...
		if (remoteDevice != null) {
			...
			for (int i = 0; i < serviceRecordCount; i++) {
				int serviceRecordHandle = (int) (((int) dataPacket[9 + (i * 4)] & 0xff) << 24
						| ((int) dataPacket[10 + (i * 4)] & 0xff) << 16 | ((int) dataPacket[11 + (i * 4)] & 0xff) << 8 | ((int) dataPacket[12 + (i * 4)] & 0xff));
				...
				if (serviceRecord == null)
					serviceRecord = new SDPRemoteServiceRecord(remoteDevice, serviceRecordHandle);
			...
			if (discoveryListener != null) {
				if (serviceRecordCount == 0)
					discoveryListener
							.serviceSearchCompleted(transactionID, DiscoveryListener.SERVICE_SEARCH_NO_RECORDS);
				else {
	...
	private void receive_SDP_ServiceAttributeResponse(byte[] dataPacket) {
		...
		if (remoteDevice != null) {
			...
			while (attributeElements.hasMoreElements()) {
				...
				if (attrID == 0) {
					serviceRecord = (SDPRemoteServiceRecord) remoteDevice.serviceRecords.get(new Integer(
							(int) attrValueElement.getLong()));
				}
				if (serviceRecord != null)
					serviceRecord.setAttributeValue(attrID, attrValueElement);
			}
			if (serviceRecord != null)
	...
	private void receive_SDP_ServiceSearchAttributeResponse(byte[] dataPacket) {
		Debug.println(4, "SDP: Received Service Attribute Response:", dataPacket);

		if ((remoteDevice != null)) {

			short transactionID = (short) (((short) dataPacket[1] & 0xff) << 8 | ((short) dataPacket[2] & 0xff));
			short attributeListByteCount = (short) (((short) dataPacket[5] & 0xff) << 8 | ((short) dataPacket[6] & 0xff));
			int continuationInfoLength = dataPacket[7 + attributeListByteCount] & 0xff;
			if (continuationInfoLength != 0) {
				Debug.println(0, "SDP: Received Service Attribute Response: partial responses: "
						+ continuationInfoLength, dataPacket);
			} else {
...
class ServiceSearchAttributeTransaction {
	
	public byte[] dataPacket;
	public byte[] request;
	public Vector responses = new Vector();

	public static byte[] buildRequest(short transactionID, short maxAttributeByteCount, DataElement uuidListElement,
			DataElement attributeListElement, byte[] continuationState) throws IOException {
		byte[] uuidList = uuidListElement.toByteArray();
		byte[] attributeList = attributeListElement.toByteArray();
		short length = (short) (uuidList.length + attributeList.length + 3);
		byte[] serviceSearchAttributeRequest = new byte[length + 5];
		...
		serviceSearchAttributeRequest[6 + uuidList.length] = (byte) ((maxAttributeByteCount) & 0xff);
		//serviceSearchAttributeRequest[7 + uuidList.length + attributeList.length] = 0x00; //no continuation
		for (int i = 0; i < continuationState.length; i++) {
			serviceSearchAttributeRequest[7 + uuidList.length + attributeList.length + i] = continuationState[i];
		}
		System.arraycopy(uuidList, 0, serviceSearchAttributeRequest, 5, uuidList.length);
		System.arraycopy(attributeList, 0, serviceSearchAttributeRequest, 7 + uuidList.length,
						attributeList.length);
		Debug.println(4, "SDP: Build Search Service Attribute Request. ", serviceSearchAttributeRequest);
