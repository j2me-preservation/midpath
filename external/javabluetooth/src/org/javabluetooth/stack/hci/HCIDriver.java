...
public abstract class HCIDriver {
	private static final byte PACKET_TYPE_COMMAND = 0x01;
	...
	public static void init(HCIDriver hciTransport) {
		HCIDriver.hciDriver = hciTransport;
	}
	...
	public void registerHCIReceiver(HCIReceiver receiver) {
		if (!hciReceivers.contains(receiver))
			hciReceivers.addElement(receiver);
	}

	public void unregisterHCIReceiver(HCIReceiver receiver) {
		hciReceivers.removeElement(receiver);
	}
	...
	public L2CAPLink getL2CAPLink(long remoteAddress, byte pageScanRepMode, byte pageScanMode, short clockOffset)
			throws HCIException {
		Long remoteAddressLong = new Long(remoteAddress);
		L2CAPLink link = (L2CAPLink) remoteAddresses.get(remoteAddressLong);
		if (link == null) {
			byte connResult = send_HCI_LC_Create_Connection(remoteAddress, (short) 0x8000, pageScanRepMode,
					pageScanMode, clockOffset, (byte) 0x01);
			if (connResult != 0)
				throw new HCIException("Create Connection failed. (" + connResult + ")");
			short timeout = 0;
			while (link == null) {
				try {
					Thread.sleep(1000);
	...
	public byte send_HCI_LC_Create_Connection(long bd_addr, short packetType, byte pageScanRepMode, byte pageScanMode,
		...
		byte[] data = { 0x01, 0x05, 0x04, 0x0d, (byte) ((bd_addr) & 0xff), (byte) ((bd_addr >> 8) & 0xff),
				(byte) ((bd_addr >> 16) & 0xff), (byte) ((bd_addr >> 24) & 0xff), (byte) ((bd_addr >> 32) & 0xff),
				(byte) ((bd_addr >> 40) & 0xff), (byte) ((packetType) & 0xff), (byte) ((packetType >> 8) & 0xff),
				(byte) pageScanRepMode, // Page scan repetition mode 1 byte
				(byte) pageScanMode, // Page scan mode 1 byte
				(byte) ((clockOffset) & 0xff), // Clock offset 2 bytes
				(byte) ((clockOffset >> 8) & 0xff), (byte) allowRoleSwitching }; // Allow role switch
	...
	public byte send_HCI_LC_Disconnect(short connectionHandle) throws HCIException {
		byte[] data = { 0x01, 0x06, 0x04, 03, (byte) ((connectionHandle) & 0xff),
				(byte) ((connectionHandle >> 8) & 0xff), 0x13 };
		byte[] resultData = send_HCI_Command_Packet(data);
		return resultData[3];
	}
	...
	protected void receiveData(byte[] incomingBytes, int incomingLength) {
		...
		if (headerBufferIndex == 0) {
			...
			case PACKET_TYPE_ACL:
				System.out.println("[DEBUG] PACKET_TYPE_ACL. headerBufferLength = 5");
				headerBufferLength = 5;
				break;
			case PACKET_TYPE_EVENT:
				System.out.println("[DEBUG] PACKET_TYPE_ACL. headerBufferLength = 4");
				headerBufferLength = 3;
				break;
			case PACKET_TYPE_COMMAND:
				System.out.println("[DEBUG] Command sent by the controller. skip.");
				return;
			default:
		...
		{
			if (headerBufferIndex < headerBufferLength) //header is still incomplete
			{
				int length = headerBufferLength - headerBufferIndex;
				if (length > incomingLength - incomingBytesIndex)
					length = incomingLength - incomingBytesIndex;
				...
				if (headerBufferIndex == headerBufferLength) //header is complete
				{
	...
	private void parseHeader() {
		...
		case PACKET_TYPE_ACL:
			...
			System.arraycopy(headerBuffer, 0, packetBuffer, 0, headerBufferLength);
			packetBufferIndex = headerBufferLength;
			break;
		case PACKET_TYPE_EVENT:
		...
		Debug.println(0, "HCI: Parsed Header " + Debug.printByteArray(headerBuffer));
	}
	...
	private void dispatchPacket(byte[] hciPacket) {
		...
		case PACKET_TYPE_EVENT:
			...
			case HCI_EVENT_INQUIRY_RESULT_WITH_RSSI:
				receive_HCI_Event_Inquiry_Result_With_RSSI(hciPacket);
				break;
			...
			case HCI_EVENT_NUMBER_OF_COMPLETED_PACKETS:
				Debug.println(1, "HCI: Received Number Of Completed Packets Event.");
				break;
			default:
			...
			break;
		case PACKET_TYPE_ACL:
			Debug.println(0, "HCI: Received Data Packet:" + Debug.printByteArray(hciPacket));
	...
	private void receive_HCI_Event_Inquiry_Result_With_RSSI(byte[] eventPacket) {
		Debug.println(1, "HCI: Received Inquiry Result With RSSI Event.");
		Enumeration receivers = hciReceivers.elements();
		while (receivers.hasMoreElements()) {
			HCIReceiver receiver = (HCIReceiver) receivers.nextElement();
			receiver.receive_HCI_Event_Inquiry_Result_With_RSSI(eventPacket);
		}
	}
	...
	private synchronized void receive_HCI_Event_Command_Complete(byte[] packetData) {
		...
		//        while (commandResponse != null) {
		//        	System.out.println("[DEBUG] HCIDriver.receive_HCI_Event_Command_Complete(): before wait");
		//            try { wait(100); }
		//            catch (InterruptedException e) { }
		//            System.out.println("[DEBUG] HCIDriver.receive_HCI_Event_Command_Complete(): after wait");
		//        }
		if (opCode == commandResponseOpCode) {
			commandResponse = packetData;
		}
	}
	...
	private void receive_HCI_Data_Packet(byte[] data) {
		...
		if (link != null) {
			...
			System.err.println("HCI: Unable to deliver Data Packet. No open link with handle " + handle + ". : "
