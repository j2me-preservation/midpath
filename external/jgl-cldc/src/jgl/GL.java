...
public class GL {
	...
	protected gl_object cc;
	//protected gl_list list;
	protected int startX = 0;
	protected int startY = 0;
	
	public GL(GLContext context) {
	...
	private int translate_id(int n, int type, byte lists[]) {
		...
		case GL_3_BYTES:
			return (int) ((lists[(n << 1) + n]) << 16 | (lists[(n << 1) + n + 1]) << 8 | (lists[(n << 1) + n + 2]));
		case GL_INT:
		case GL_UNSIGNED_INT:
		case GL_4_BYTES:
			return (int) ((lists[n << 2]) << 24 | (lists[(n << 2) | 1]) << 16 | (lists[(n << 2) | 2]) << 8 | (lists[(n << 2) | 3]));
		case GL_FLOAT:
			return (int) ((lists[n << 2]) * 16777216.0f + (lists[(n << 2) | 1]) * 65536.0f + (lists[(n << 2) | 2])
					* 256.0f + (lists[(n << 2) | 3]));
		default:
			return 0;
		}
	...
	public void glClearColor(float red, float green, float blue, float alpha) {
		...
		cc.gl_clear_color(gl_util.CLAMP(red, 0.0f, 1.0f), gl_util.CLAMP(green, 0.0f, 1.0f), gl_util.CLAMP(blue, 0.0f,
				1.0f), gl_util.CLAMP(alpha, 0.0f, 1.0f));
	}
	...
	public void glPointSize(float size) {
		...
		cc.gl_point_size(gl_util.CLAMP(size, GLContext.MIN_POINT_SIZE, GLContext.MAX_POINT_SIZE));
	}
	...
	public void glLineWidth(float width) {
		...
		cc.gl_line_width(gl_util.CLAMP(width, GLContext.MIN_LINE_SIZE, GLContext.MAX_LINE_SIZE));
	}
	...
	public void glClipPlane(int plane, double equation[]) {
		...
		if (plane < GL_CLIP_PLANE0 || plane - GL_CLIP_PLANE0 >= GLContext.MAX_CLIP_PLANES) {
	...
	public void glGetFloatv(int pname, float params[]) {
		...
		case GL_STEREO:
			break;
		case GL_POINT_SIZE_RANGE:
			params[0] = GLContext.MIN_POINT_SIZE;
			params[0] = GLContext.MAX_POINT_SIZE;
			break;
		case GL_POINT_SIZE_GRANULARITY:
			params[0] = GLContext.POINT_SIZE_GRANULARITY;
			break;
		case GL_LINE_WIDTH_RANGE:
			params[0] = GLContext.MIN_LINE_SIZE;
			params[0] = GLContext.MAX_LINE_SIZE;
			break;
		case GL_LINE_WIDTH_GRANULARITY:
			params[0] = GLContext.LINE_WIDTH_GRANULARITY;
			break;
		default:
	...
	public void glGetIntegerv(int pname, int params[]) {
		...
		case GL_FOG_HINT:
			break;
		case GL_MAX_LIGHTS:
			params[0] = GLContext.MAX_LIGHTS;
			break;
		case GL_MAX_CLIP_PLANES:
			params[0] = GLContext.MAX_CLIP_PLANES;
			break;
		...
		case GL_MAX_TEXTURE_SIZE:
			params[0] = GLContext.MAX_TEXTURE_SIZE;
			break;
		case GL_MAX_3D_TEXTURE_SIZE:
			params[0] = GLContext.MAX_3D_TEXTURE_SIZE;
			break;
		case GL_MAX_PIXEL_MAP_TABLE:
			break;
		case GL_MAX_NAME_STACK_DEPTH:
			params[0] = GLContext.MAX_NAME_STACK_DEPTH;
			break;
		case GL_MAX_LIST_NESTING:
			break;
		case GL_MAX_EVAL_ORDER:
			params[0] = GLContext.MAX_EVAL_ORDER;
			break;
	...
	public void glFlush() {
		...
		context.getBackend().updatePixels(context.viewport.Width, context.viewport.Height, context.colorBuffer.buffer, 0,
	...
	public void glLoadMatrixd(double m[]) {
		...
		float temp[] = { (float) m[0], (float) m[1], (float) m[2], (float) m[3], (float) m[4], (float) m[5],
				(float) m[6], (float) m[7], (float) m[8], (float) m[9], (float) m[10], (float) m[11], (float) m[12],
...
//		}
	...
	public void glColor3d(double red, double green, double blue) {
		cc.gl_color(gl_util.CLAMP(red, 0.0, 1.0), gl_util.CLAMP(green, 0.0, 1.0), gl_util.CLAMP(blue, 0.0, 1.0), 1.0f);
	}
	...
	public void glColor3i(int red, int green, int blue) {
		cc.gl_color(gl_util.ItoF(gl_util.CLAMP(red, 0, 255)), gl_util.ItoF(gl_util.CLAMP(green, 0, 255)), gl_util
				.ItoF(gl_util.CLAMP(blue, 0, 255)), 1.0f);
	}
	...
	public void glColor4d(double red, double green, double blue, double alpha) {
		cc.gl_color(gl_util.CLAMP(red, 0.0, 1.0), gl_util.CLAMP(green, 0.0, 1.0), gl_util.CLAMP(blue, 0.0, 1.0),
	...
	public void glColor4i(int red, int green, int blue, int alpha) {
		cc.gl_color(gl_util.ItoF(gl_util.CLAMP(red, 0, 255)), gl_util.ItoF(gl_util.CLAMP(green, 0, 255)), gl_util
				.ItoF(gl_util.CLAMP(blue, 0, 255)), gl_util.ItoF(gl_util.CLAMP(alpha, 0, 255)));
	}
	...
	public void glLightf(int light, int pname, float param) {
		...
		if (light < GL_LIGHT0 || light - GL_LIGHT0 >= GLContext.MAX_LIGHTS) {
	...
	public void glTexGeniv(int coord, int pname, int params[]) {
		...
		case GL_TEXTURE_GEN_MODE:
			if ((coord == GL_S) || (coord == GL_T)) {
				if ((params[0] == GL_OBJECT_LINEAR) || (params[0] == GL_EYE_LINEAR) || (params[0] == GL_SPHERE_MAP)) {
	...
	public void glTexEnvf(int target, int pname, float param) {
		...
		if (pname == GL_TEXTURE_ENV_MODE) {
			...
			if ((iparam == GL_MODULATE) || (iparam == GL_BLEND) || (iparam == GL_DECAL) || (iparam == GL_REPLACE)) {
	...
	public void glTexEnvi(int target, int pname, int param) {
		...
		if (pname == GL_TEXTURE_ENV_MODE) {
			if ((param == GL_MODULATE) || (param == GL_BLEND) || (param == GL_DECAL) || (param == GL_REPLACE)) {
	...
	public void glTexImage1D(int target, int level, int components, int width, int border, int format, int type,
			Object pixels) {
		if (cc.Mode != None) {
		...
		if (level < 0 || level >= GLContext.MAX_TEXTURE_LEVELS) {
		...
		if (width < 2 * border || width > 2 + GLContext.MAX_TEXTURE_SIZE) {
	...
	//			      int format, int type, byte pixels [][][]) {
		...
		if (height < 2 * border || height > 2 + GLContext.MAX_TEXTURE_SIZE) {
	...
	/** GLvoid glTexImage3D (GLenum target, GLint level, GLint components,
	 ...
	 GLenum format, GLenum type, const GLvoid *pixels) */
	public void glTexImage3D(int target, int level, int components, int width, int height, int depth, int border,
			int format, int type, Object pixels) {
		if (cc.Mode != None) {
		...
		if (depth < 2 * border || depth > 2 + GLContext.MAX_3D_TEXTURE_SIZE) {
	...
	public void glTexSubImage2D(int target, int level, int xoffset, int yoffset, int width, int height, int format,
		...
		cc.gl_tex_sub_image_2d(target, level, xoffset + b, yoffset + b, width + xoffset + b, height + yoffset + b,
				format, size, pixels);
	}
	...
	public void glTexSubImage3D(int target, int level, int xoffset, int yoffset, int zoffset, int width, int height,
			int depth, int format, int type, Object pixels) {
		if (target != GL_TEXTURE_3D) {
		...
		cc.gl_tex_sub_image_3d(target, level, xoffset + b, yoffset + b, zoffset + b, width + xoffset + b, height
				+ yoffset + b, depth + zoffset + b, format, size, pixels);
	}
	...
	public void glMap1d(int target, double u1, double u2, int stride, int order, double points[][]) {
		...
		if (order < 1 || order > GLContext.MAX_EVAL_ORDER) {
	...
	public void glMap2d(int target, double u1, double u2, int ustride, int uorder, double v1, double v2, int vstride,
		...
		if (uorder < 1 || uorder > GLContext.MAX_EVAL_ORDER) {
			cc.gl_error(GL_INVALID_VALUE, "glMap2d(uorder)");
			return;
		}
		if (vorder < 1 || vorder > GLContext.MAX_EVAL_ORDER) {
		...
		if (dim == 0) {
			...
			context.gl_map_2(target, (float) u1, (float) u2, ustride, uorder, (float) v1, (float) v2, vstride, vorder,
					np);
		}
		if (ustride < dim) {
	...
	public boolean glXMakeCurrent(GLContext context, GLBackend backend) {
		...
		glViewport(backend.getX(), backend.getY(), backend.getWidth(), backend.getHeight());
...
//	}
	...
	public void glXSwapBuffers() {
		context.getBackend().sync();
