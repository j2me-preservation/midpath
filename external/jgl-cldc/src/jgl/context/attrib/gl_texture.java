...
import jgl.context.util.MathExt;
...
public class gl_texture {
	...
	private int tex_map_linear_1d(float s, int l) {
		...
		rgb[0] = (int) (((float) (img.ImageData[i[0]][0][0][0] & 0x000000ff)) * w000 + ((float) (img.ImageData[i[1]][0][0][0] & 0x000000ff))
				* w001);
		rgb[1] = (int) (((float) (img.ImageData[i[0]][0][0][1] & 0x000000ff)) * w000 + ((float) (img.ImageData[i[1]][0][0][1] & 0x000000ff))
				* w001);
	...
	private int tex_map_linear_2d(float s, float t, int l) {
		...
		rgb[0] = (int) (((float) (img.ImageData[i[0]][j[0]][0][0] & 0x000000ff)) * w000
				...
				+ ((float) (img.ImageData[i[0]][j[1]][0][0] & 0x000000ff)) * w010 + ((float) (img.ImageData[i[1]][j[1]][0][0] & 0x000000ff))
				* w011);
		rgb[1] = (int) (((float) (img.ImageData[i[0]][j[0]][0][1] & 0x000000ff)) * w000
				+ ((float) (img.ImageData[i[1]][j[0]][0][1] & 0x000000ff)) * w001
				+ ((float) (img.ImageData[i[0]][j[1]][0][1] & 0x000000ff)) * w010 + ((float) (img.ImageData[i[1]][j[1]][0][1] & 0x000000ff))
				* w011);
	...
	private int tex_map_linear_3d(float s, float t, float r, int l) {
		...
		rgb[0] = (int) (((float) (img.ImageData[i[0]][j[0]][k[0]][0] & 0x000000ff)) * w000
				...
				+ ((float) (img.ImageData[i[0]][j[1]][k[1]][0] & 0x000000ff)) * w110 + ((float) (img.ImageData[i[1]][j[1]][k[1]][0] & 0x000000ff))
				* w111);
		rgb[1] = (int) (((float) (img.ImageData[i[0]][j[0]][k[0]][1] & 0x000000ff)) * w000
				...
				+ ((float) (img.ImageData[i[0]][j[1]][k[1]][1] & 0x000000ff)) * w110 + ((float) (img.ImageData[i[1]][j[1]][k[1]][1] & 0x000000ff))
				* w111);
	...
	private int tex_mid(float s, float t, float r, float l) {
		...
		if (CurrentObj.MagFilter == GL.GL_LINEAR
				&& (CurrentObj.MinFilter == GL.GL_NEAREST_MIPMAP_NEAREST || CurrentObj.MinFilter == GL.GL_LINEAR_MIPMAP_NEAREST)) {
			c = (float) 0.5;
		} else {
			c = 0;
		}
	...
	private float lambda(float w, float dsdx, float dsdy, float dtdx, float dtdy, float drdx, float drdy) {
		...
		return ((float) (MathExt.log(rho) * 1.442695)); // 1.442695=1/log2
	}

	//    public int tex_vertex (float s, float t, float w, float dsdx, float dsdy,
	//						      float dtdx, float dtdy) {
	public int tex_vertex(float s, float t, float r, float w, float dsdx, float dsdy, float dtdx, float dtdy,
