 ...
 */
final class NativeEGLDisplay extends EGLDisplay {

	private static final Hashtable byId = new Hashtable();
	int nativeId;

	public NativeEGLDisplay(int nativeId) {
		synchronized (byId) {
	...
	public static NativeEGLDisplay getInstance(int nativeId) {
		synchronized (byId) {
			WeakReference ref = (WeakReference) byId.get(new Integer(nativeId));
			NativeEGLDisplay display = ref != null ? (NativeEGLDisplay) ref.get() : null;
			if (display == null) {
				return new NativeEGLDisplay(nativeId);
			} else {
				return display;
			}
		}
