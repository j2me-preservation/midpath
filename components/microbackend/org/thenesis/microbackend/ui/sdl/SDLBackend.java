...
public class SDLBackend implements UIBackend {
	...
	public void initialize(Configuration conf, int width, int height) {
		...
		bitsPerPixel = conf.getIntParamater("org.thenesis.microbackend.ui.sdl.bitsPerPixel", 32);
	...
	public void open() throws IOException {
		...
		try {
			...
			rootARGBSurface = SDLVideo.createRGBSurface(SDLVideo.SDL_SWSURFACE, canvasWidth, canvasHeight, 32,
					0x00ff0000L, 0x0000ff00L, 0x000000ffL, 0xff000000L);
			if (Logging.TRACE_ENABLED)
	...
	public class SDLEventThread implements Runnable {
		...
		public void processEvent(SDLKeyboardEvent event) {
			...
			if (event.getState() == SDLPressedState.PRESSED) {
				if (Logging.TRACE_ENABLED)
					System.out.println("[DEBUG] SDLEventThread.processEvent(): keyCode: " + keyCode + " char: "
							+ (char) unicode);
