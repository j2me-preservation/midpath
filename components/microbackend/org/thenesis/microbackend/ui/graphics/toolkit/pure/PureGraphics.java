...
public class PureGraphics implements VirtualGraphics {
    ...
    PureGraphics(VirtualSurface surface) {
     ...
     */
    public void drawRegion(PureImage src, 
                                  int x_src, int y_src,
     ...
     */
    void resetGC() {
        currentFont = new PureFont(VirtualFont.FACE_MONOSPACE, VirtualFont.STYLE_PLAIN, VirtualFont.SIZE_LARGE);
        style       = SOLID;
        rgbColor    = gray = 0;
        pixel       = getPixel(rgbColor, gray, true);
    }
    ...
    public void drawHLine(int x0, int x1, int y) {
        ...
        // Draw.
        drawSpan(surface.getData(), y * surface.getWidth() + x0, x1 - x0 + 1);
    }
    ...
    public void drawVLine(int y0, int y1, int x) {
        ...
        for (int ry = 0; ry < h; ry++)
            drawSpan(surface.getData(), (y0 + ry) * surface.getWidth() + x, 1);
    }
        ...
        // Draw.
        {
            ...
            if (dx >= dy) {
                ...
                if (lastPixelFlag)
                    w++;
                while (w > 0) {
                    drawSpan(surface.getData(), y * surface.getWidth() + x, 1);
                    if (decision >= 0) {
                        decision -= dx;
                        y += sy;
                    }
    ...
    public void fillRect(int x0, int y0, int width, int height) {
        ...
        for (int ry = 0; ry < h; ry++) {
            drawSpan(surface.getData(), (y0 + ry) * surface.getWidth() + x0, w);
        }
    }
    ...
    private void drawQuadrantPoints(int x, int y, int xp, int yp) {
        ...
        if ((y0 >= r.ymin) && (y0 <= r.ymax)) {
            if (left)
                drawSpan(surface.getData(), y0 * surface.getWidth() + x0, 1);
            if (right)
                drawSpan(surface.getData(), y0 * surface.getWidth() + x1, 1);
        }
        if ((y1 >= r.ymin) && (y1 <= r.ymax) && (yp != 0)) {
            if (left)
                drawSpan(surface.getData(), y1 * surface.getWidth() + x0, 1);
            if (right)
                drawSpan(surface.getData(), y1 * surface.getWidth() + x1, 1);
        }
    }
    ...
    private void drawQuadrantSpans(int x, int y, int xp, int yp) {
        ...
        if (w > 0) {
            if ((y0 >= r.ymin) && (y0 <= r.ymax))
                drawSpan(surface.getData(), y0 * surface.getWidth() + x0, w);
            if ((y1 >= r.ymin) && (y1 <= r.ymax) && (yp != 0))
                drawSpan(surface.getData(), y1 * surface.getWidth() + x0, w);
        }
    }
    ...
    private void drawArcPoints(int x, int y, int xp, int yp, int xs, int ys, int xe, int ye, boolean reflexAngle) {
        ...
        if ((y0 >= r.ymin) && (y0 <= r.ymax)) {
            if (left) {
                ...
                if (insideFlag)
                    drawSpan(surface.getData(), y0 * surface.getWidth() + x0, 1); // Top
                // left.
                // Second
                // quadrant
                // .
            ...
            if (right) {
                insideFlag = reflexAngle ? (xpys - ypxs < 0 || xpye - ypxe > 0) : (xpys - ypxs < 0 && xpye - ypxe > 0);
                if (insideFlag)
                    drawSpan(surface.getData(), y0 * surface.getWidth() + x1, 1); // Top
                // right
                // .
                // First
                // quadrant
        ...
        if ((y1 >= r.ymin) && (y1 <= r.ymax) && (yp != 0)) {
            if (left) {
                insideFlag = reflexAngle ? (-xpys + ypxs < 0 || -xpye + ypxe > 0) : (-xpys + ypxs < 0 && -xpye + ypxe > 0);
                if (insideFlag)
                    drawSpan(surface.getData(), y1 * surface.getWidth() + x0, 1); // Bottom
                // left.
                // Third
                // quadrant
                // .
            ...
            if (right) {
                insideFlag = reflexAngle ? (xpys + ypxs < 0 || xpye + ypxe > 0) : (xpys + ypxs < 0 && xpye + ypxe > 0);
                if (insideFlag)
                    drawSpan(surface.getData(), y1 * surface.getWidth() + x1, 1); // Bottom
                // right
                // .
                // Fourth
                // quadrant
    ...
    public void drawRGB(int[] rgbData, int offset, int scanlength, int x, int y, int width, int height, boolean processAlpha) {
        ...
        if (processAlpha) {
            for (int ry = 0; ry < h; ry++) {
                ...
                int dstPosition = (dymin + ry) * surface.getWidth() + dxmin;
                int length = w;
    ...
    protected void doCopyArea(int x_src, int y_src, int width, int height, int x_dest, int y_dest, int anchor) {
        ...
        PureImage image = new PureImage(width, height);
        int[] srcSurfaceData = surface.getData();
        int[] destSurfaceData = image.surface.getData();

        for (int j = 0; j < height; j++) {

            int srcPosition = (y_src + j) * surface.getWidth() + x_src;
