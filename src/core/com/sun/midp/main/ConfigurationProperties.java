...
/*
 * EBNF for parsing configuration
...
/**
 * This class represents a set of properties loaded from a configuration
 * file.
 * <p>
...
public class ConfigurationProperties extends Properties {
	 ...
	 * @exception  InvalidConfigurationException if the JAD is not formatted correctly.
	 */
	public void partialLoad(InputStream inStream, String enc, int propertiesToLoad) throws IOException,
			IOException {
		Reader in;
		String line;
		int endOfKey;
		...
		int startOfValue;
		String value = null;
		IOException configException = null;
		...
		for (int i = 0; i < propertiesToLoad; i++) {
			...
			if (endOfKey == -1) {
				configException = new IOException("Invalid key : " + line);
				continue;
			}
			...
			value = value.trim();
			if (value == null) {
				configException = new IOException("Invalid value : " + key);
				continue;
			}
	...
	protected boolean checkKeyChars(String key) {
		...
		for (int i = 0; i < len; i++) {
			...
			if (current <= 0x1F || current == 0x7F || current == '(' || current == ')' || current == '<'
					|| current == '>' || current == '@' || current == ',' || current == ';' || current == '\''
					|| current == '"' || current == '/' || current == '[' || current == ']' || current == '?'
					|| current == '=' || current == '{' || current == '}' || current == SP || current == HT) {
