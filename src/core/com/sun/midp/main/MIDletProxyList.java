...
public class MIDletProxyList implements MIDletControllerEventConsumer {
	...
	MIDletProxyList(EventQueue eventQueue) {
		...
		new MIDletControllerEventListener(eventQueue, (MIDletControllerEventConsumer) this);
	}
	...
	private MIDletProxy findMIDletProxy(int isolateId, int displayId) {
		synchronized (midletProxies) {
			for (int i = midletProxies.size() - 1; i >= 0; i--) {
				...
				if (current.getIsolateId() == isolateId && current.getDisplayId() == displayId) {
					return current;
				}
			}
		}
	...
	public MIDletProxy findMIDletProxy(String suiteID, String classname) {
		synchronized (midletProxies) {
			for (int i = midletProxies.size() - 1; i >= 0; i--) {
				...
				if (current.getSuiteId().equals(suiteID) && current.getClassName().equals(classname)) {
					return current;
				}
			}
		}
	 ...
	 */
	public void handleMIDletCreateNotifyEvent(int midletExternalAppId, int midletIsolateId, int midletDisplayId,
			String midletSuiteId, String midletClassName, String midletDisplayName) {

		MIDletProxy midletProxy = findMIDletProxy(midletIsolateId, midletDisplayId);
		if (midletProxy != null) {
			/**
		...
		midletProxy = new MIDletProxy(this, midletExternalAppId, midletIsolateId, midletDisplayId, midletSuiteId,
				midletClassName, midletDisplayName, MIDletProxy.MIDLET_PAUSED);
	 ...
	 * @param midletDisplayId ID of the sending Display
	 */
	public void handleMIDletActiveNotifyEvent(int midletIsolateId, int midletDisplayId) {
		...
		notifyListenersOfProxyUpdate(midletProxy, MIDletProxyListListener.MIDLET_STATE);
	...
	// MIDletProxy midletProxy) {
			int midletIsolateId, int midletDisplayId, boolean isAlert) {
		...
		if (midletProxy == foregroundMidlet) {
			...
			setForegroundMIDlet(displayController.foregroundRequest(midletProxy));
		}
		 ...
		 * setForegroundMIDlet().
		 */
		notifyListenersOfProxyUpdate(midletProxy, MIDletProxyListListener.WANTS_FOREGROUND);
	}
	 ...
	 * @param midletDisplayId ID of the sending Display
	 */
	public void handleDisplayPreemptStartEvent(int midletIsolateId, int midletDisplayId) {

		MIDletProxy preempting = new MIDletProxy(this, 0, midletIsolateId, midletDisplayId, null, null, null,
				MIDletProxy.MIDLET_ACTIVE);

		MIDletProxy nextForeground = displayController.startPreempting(preempting);
	...
	public void handleDisplayPreemptStopEvent(int midletIsolateId, int midletDisplayId) {
		...
		MIDletProxy nextForeground = displayController.endPreempting(midletIsolateId, midletDisplayId);
	 ...
	 *        to give forefround ownership to
	 */
	public void handleMIDletForegroundTransferEvent(String originMIDletSuiteId, String originMIDletClassName,
			String targetMIDletSuiteId, String targetMIDletClassName) {

		MIDletProxy origin = findMIDletProxy(originMIDletSuiteId, originMIDletClassName);
		if (origin == null) {
			return;
		}
		// See if the foreground can be handed to the target MIDlet
		MIDletProxy target = findMIDletProxy(targetMIDletSuiteId, targetMIDletClassName);
		if (target != null) {
			target.setWantsForeground(true, false);
			// Let the DisplayController make the UE policy choice
			setForegroundMIDlet(displayController.transferRequest(origin, target));
			 ...
			 * setForegroundMIDlet().
			 */
			notifyListenersOfProxyUpdate(target, MIDletProxyListListener.WANTS_FOREGROUND);
		}
	}
	 ...
	 */
	public void handleSetForegroundByNameRequestEvent(String suiteId, String className) {
	 ...
	 */
	public void handleMIDletStartErrorEvent(int midletExternalAppId, String midletSuiteId, String midletClassName,
			int error) {

		for (int i = listeners.size() - 1; i >= 0; i--) {
			MIDletProxyListListener listener = (MIDletProxyListListener) listeners.elementAt(i);

			listener.midletStartError(midletExternalAppId, midletSuiteId, midletClassName, error);
		}
	}
	 ...
	 * @param midletDisplayId ID of the sending Display
	 */
	public void handleMIDletResumeRequestEvent(int midletIsolateId, int midletDisplayId) {
		MIDletProxy midletProxy = findMIDletProxy(midletIsolateId, midletDisplayId);
		if (midletProxy == null) {
			/*
			 * There is nothing we can do for the event
	...
	public void setForegroundMIDlet(MIDletProxy newForeground) {
		if (newForeground != null
				&& (newForeground.getMidletState() == MIDletProxy.MIDLET_DESTROYED || newForeground == foregroundMidlet)) {
			return;
		}

		if (foregroundMidlet != null && (foregroundMidlet.getMidletState() != MIDletProxy.MIDLET_DESTROYED)) {
			/*
		...
		foregroundMidlet = displayController.foregroundMidletChanging(newForeground);
		...
		if (foregroundMidlet != null) {
			setForegroundInNativeState(foregroundMidlet.getIsolateId(), foregroundMidlet.getDisplayId());
	 ...
	 */
	void notifyListenersOfProxyListChange(MIDletProxy midletProxy, int notificationType) {
	 ...
	 */
	void notifyListenersOfProxyUpdate(MIDletProxy midletProxy, int reason) {
	...
	void removeMidletProxy(MIDletProxy midletProxy) {
		...
		if (preempting != null) {
			...
			setForegroundMIDlet(displayController.midletDestroyed(midletProxy));
		}
	 ...
	 * @param displayId Display ID
	 */
	private void setForegroundInNativeState(int isolateId, int displayId) {
		if (Logging.TRACE_ENABLED)
			System.out.println("[DEBUG] MIDletProxyList.setForegroundInNativeState(): not yet implemented");
	}

	/**
	 * Notify native code that all MIDlets have been paused.
	 */
	private void notifySuspendAll0() {
		if (Logging.TRACE_ENABLED)
			System.out.println("[DEBUG] MIDletProxyList.notifySuspendAll0(): not yet implemented");
	}

	/**
	 * Notify native code that all MIDlets have been resumed.
	 */
	private void notifyResumeAll0() {
		if (Logging.TRACE_ENABLED)
			System.out.println("[DEBUG] MIDletProxyList.notifyResumeAll0(): not yet implemented");
	}
}
