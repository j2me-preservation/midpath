...
//                midletSuite);
    ...
    public void startSuite(MIDletSuite aMidletSuite, int externalAppId,
        ...
        while (nmidlets > 0) {
            try {
                ...
                synchronized (this) {
                    ...
                    if (Logging.TRACE_ENABLED)
                    	System.out.println("[DEBUG] MidletStateHandler.startSuite() : state: " + state );
                    switch (state) {
                    ...
                    case MIDletPeer.PAUSED:
                        ...
                        try {
                        	if (Logging.TRACE_ENABLED)
                        		System.out.println("[DEBUG] MidletStateHandler.startSuite() : before wait: " );
                            this.wait();
                            if (Logging.TRACE_ENABLED)
                            	System.out.println("[DEBUG] MidletStateHandler.startSuite() : after wait: " );
                ...
                case MIDletPeer.ACTIVE_PENDING:
                    try {
                        curr.startApp();
                        if (Logging.TRACE_ENABLED)
                        	System.out.println("[DEBUG] MidletStateHandler.startSuite(): ACTIVE_PENDING");
                    } catch (Throwable ex) {
                        if (Logging.TRACE_ENABLED) {
            		...
            		//NativeEvent nativeEvent = new NativeEvent(EventTypes.FOREGROUND_NOTIFY_EVENT);
            ...
            if (Logging.TRACE_ENABLED)
            	System.out.println("[DEBUG] MidletStateHandler.startSuite() 2"  );
        }
        if (Logging.TRACE_ENABLED)
        	System.out.println("[DEBUG] MidletStateHandler.startSuite() 2"  ); //+displayEventHandler.getClass());
