...
import org.thenesis.midpath.mmedia.TonePlayer;
import org.thenesis.midpath.mmedia.protocol.BasicDS;
import org.thenesis.midpath.mmedia.protocol.CommonDS;
...
public final class Manager {
	...
	private final static String PL_ERR_SH = "Cannot create a Player: ";
	...
	public static Player createPlayer(String locator) throws IOException, MediaException {
		 ...
		 */
		if (locStr.equals(validLoc = TONE_DEVICE_LOCATOR) || locStr.equals(validLoc = MIDI_DEVICE_LOCATOR)
				|| locStr.startsWith(validLoc = Configuration.VIDEO_CAPTURE_LOCATOR)
				|| locStr.startsWith(validLoc = Configuration.AUDIO_CAPTURE_LOCATOR)) {
	...
	private static BasicPlayer getPlayerFromType(String type) throws IOException, MediaException {
		...
		if ((type == null) || (className = config.getHandler(type)) == null) {
			throw new MediaException(PL_ERR_SH + "Mime type " + type + " is not supported");
		}
	 ...
	 * @exception  MediaException  Description of the Exception
	 */
	private static Player createPlayer(DataSource source) throws IOException, MediaException {
		if (source == null) {
			throw new IllegalArgumentException();
		}
