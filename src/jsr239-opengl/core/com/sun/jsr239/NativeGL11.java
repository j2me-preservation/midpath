...
public class NativeGL11 extends NativeGL10 implements GL11, GL11Ext, GL11ExtensionPack {
	...
	public synchronized void glGetBufferParameteriv(int target, int pname, IntBuffer params) {
		...
		if (!params.isDirect()) {
			...
			IglGetBufferParameteriv(target, pname, null, pointer(params), length);
		}
	}
	...
	public synchronized void glColorPointer(int size, int type, int stride, int offset) {
		...
		if (VBOArrayBufferBound == 0) {
			throw new IllegalStateException("glColorPointer:" + Errors.VBO_ARRAY_BUFFER_UNBOUND);
		}
		...
		if ((size == 4) && (type == GL_UNSIGNED_BYTE || type == GL_FIXED || type == GL_FLOAT) && (stride >= 0)) {
	...
	public synchronized void glNormalPointer(int type, int stride, int offset) {
		checkThread();
		if (VBOArrayBufferBound == 0) {
			throw new IllegalStateException("glNormalPointer:" + Errors.VBO_ARRAY_BUFFER_UNBOUND);
		}
		...
		if ((type == GL_BYTE || type == GL_SHORT || type == GL_FIXED || type == GL_FLOAT) && (stride >= 0)) {
	...
	public synchronized void glTexCoordPointer(int size, int type, int stride, int offset) {
		checkThread();
		if (VBOArrayBufferBound == 0) {
			throw new IllegalStateException("glTexCoordPointer:" + Errors.VBO_ARRAY_BUFFER_UNBOUND);
		}
		...
		if ((size >= 2 && size <= 4) && (type == GL_BYTE || type == GL_SHORT || type == GL_FIXED || type == GL_FLOAT)
				&& (stride >= 0)) {
	...
	public synchronized void glVertexPointer(int size, int type, int stride, int offset) {
		checkThread();
		if (VBOArrayBufferBound == 0) {
			throw new IllegalStateException("glVertexPointer:" + Errors.VBO_ARRAY_BUFFER_UNBOUND);
		}
	...
	public synchronized void glPointSizePointerOES(int type, int stride, Buffer pointer) {
		checkThread();
		if (VBOArrayBufferBound != 0) {
			throw new IllegalStateException("glPointSizePointerOES:" + Errors.VBO_ARRAY_BUFFER_BOUND);
		}
		if (pointer == null) {
		...
		// Only record details if this is a legal operation
		if ((type == GL_FIXED || type == GL_FLOAT) && (stride >= 0)) {
	...
	public synchronized void glPointSizePointerOES(int type, int stride, int offset) {
		checkThread();
		if (VBOArrayBufferBound == 0) {
			throw new IllegalStateException("glPointSizePointerOES:" + Errors.VBO_ARRAY_BUFFER_UNBOUND);
		}
	...
	public synchronized void glDrawElements(int mode, int count, int type, int offset) {
		...
		if (VBOElementArrayBufferBound == 0) {
			throw new IllegalStateException("glDrawElements:" + Errors.VBO_ELEMENT_ARRAY_BUFFER_UNBOUND);
		}
	...
	public synchronized void glMatrixIndexPointerOES(int size, int type, int stride, Buffer pointer) {
		...
		if (VBOArrayBufferBound != 0) {
			throw new IllegalStateException("glMatrixIndexPointerOES:" + Errors.VBO_ARRAY_BUFFER_BOUND);
		}
		if (!isDirect(pointer)) {
			throw new ArrayIndexOutOfBoundsException(Errors.GL_NOT_DIRECT);
		}

		if ((size <= GLConfiguration.MAX_VERTEX_UNITS) && (type == GL_UNSIGNED_BYTE) && (stride >= 0)) {
			...
			int nbytes = bufferTypeSize(pointer);
			pointerRemaining[MATRIX_INDEX_POINTER] = pointer.remaining() * nbytes;
	...
	public synchronized void glMatrixIndexPointerOES(int size, int type, int stride, int offset) {
		...
		if (VBOArrayBufferBound == 0) {
			throw new IllegalStateException("glMatrixIndexPointerOES:" + Errors.VBO_ARRAY_BUFFER_UNBOUND);
		}
		...
		if ((size > 0) && (size <= GLConfiguration.MAX_VERTEX_UNITS) && (type == GL_UNSIGNED_BYTE) && (stride >= 0)) {
			BufferManager.releaseBuffer(pointerBuffer[MATRIX_INDEX_POINTER]);
	...
	public synchronized void glWeightPointerOES(int size, int type, int stride, Buffer pointer) {
		...
		if (VBOArrayBufferBound != 0) {
			throw new IllegalStateException("glWeightPointerOES:" + Errors.VBO_ARRAY_BUFFER_BOUND);
		}
		if (!isDirect(pointer)) {
			throw new IllegalArgumentException(Errors.GL_NOT_DIRECT);
		}

		if ((size <= GLConfiguration.MAX_VERTEX_UNITS) && (type == GL_FIXED || type == GL_FLOAT) && (stride >= 0)) {
	...
	public synchronized void glWeightPointerOES(int size, int type, int stride, int offset) {
		...
		if (VBOArrayBufferBound == 0) {
			throw new IllegalStateException("glWeightPointerOES:" + Errors.VBO_ARRAY_BUFFER_UNBOUND);
		}
		...
		if ((size >= 0) && (size <= GLConfiguration.MAX_VERTEX_UNITS) && (type == GL_FIXED || type == GL_FLOAT)
				&& (stride >= 0)) {
			BufferManager.releaseBuffer(pointerBuffer[WEIGHT_POINTER]);
	...
	public synchronized void glGetRenderbufferParameterivOES(int target, int pname, IntBuffer params) {
		...
		if (!params.isDirect()) {
			...
			IglGetRenderbufferParameterivOES(target, pname, null, pointer(params), length);
		}
	}
	...
	public synchronized void glFramebufferRenderbufferOES(int target, int attachment, int renderbuffertarget,
			int renderbuffer) {
		checkThread();
		check_fbo();
	...
	public synchronized void glGetFramebufferAttachmentParameterivOES(int target, int attachment, int pname,
	...
	public synchronized void glGetFramebufferAttachmentParameterivOES(int target, int attachment, int pname,
		...
		if (!params.isDirect()) {
			...
			IglGetFramebufferAttachmentParameterivOES(target, attachment, pname, null, pointer(params), length);
		}
	}

	void IglGetFramebufferAttachmentParameterivOES(int target, int attachment, int pname, int[] params, int offset,
			int length) {
		grabContext();
	...
	public NativeGL11(EGLContext context) {
		super(context);
	}
}
