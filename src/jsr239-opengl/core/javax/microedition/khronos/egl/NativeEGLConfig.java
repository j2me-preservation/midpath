 ...
 */
final class NativeEGLConfig extends EGLConfig {

    private static final Hashtable byId = new Hashtable(); 
    int nativeId;

    public NativeEGLConfig(int nativeId) {
        synchronized (byId) {
    ...
    public static NativeEGLConfig getInstance(int nativeId) {
        synchronized (byId) {
            WeakReference ref = (WeakReference)byId.get(new Integer(nativeId));
            NativeEGLConfig config = ref != null ?
                    (NativeEGLConfig)ref.get() : null;
            if (config == null) {
                return new NativeEGLConfig(nativeId);
            } else {
                return config;
            }
        }
