 ...
 */
void imgj_get_argb(const gxj_screen_buffer *sbuf,
                   jint * rgbBuffer,
                   jint offset,
                   jint scanlength,
        ...
        if (srcAlphaData != NULL) {
            for (b = y; b < y + height; b++) {
                for (a = x; a < x + width; a++) {
                    ...
                    rgbBuffer[offset + (a - x) + (b - y) * scanlength] = (alpha << 24) + RGB16TORGB24(pixel);
                }
            }
        } else {
            for (b = y; b < y + height; b++) {
                for (a = x; a < x + width; a++) {
                    pixel = srcPixelData[b * srcWidth + a];
                    rgbBuffer[offset + (a - x) + (b - y) * scanlength] = RGB16TORGB24(pixel) | 0xFF000000;
                }
            }
        }
    }
...
JNIEXPORT void JNICALL Java_org_thenesis_microbackend_ui_graphics_toolkit_gxj_ImageDataFactory_loadRGB
  (JNIEnv *env, jobject obj, jobject imageData, jintArray argbIntArray) {
    ...
    if (sbuf->pixelByteArray != NULL) {
        ...
        PIXEL *pixelData = (PIXEL *)sbuf->pixelData;
        ALPHA *alphaData = (ALPHA *)sbuf->alphaData;
...
JNIEXPORT void JNICALL Java_org_thenesis_microbackend_ui_graphics_toolkit_gxj_ImageDataFactory_loadRegion
  (JNIEnv *env, jobject obj, jobject dstImg, jobject srcImg, jint x, jint y, jint width, jint height, jint transform) {

    gxj_screen_buffer srcScreenBuffer;
    gxj_screen_buffer *srcBuffer = getScreenBufferFromImageData(env, srcImg, &srcScreenBuffer);

    gxj_screen_buffer dstScreenBuffer;
    gxj_screen_buffer *dstBuffer = getScreenBufferFromImageData(env, dstImg, &dstScreenBuffer);

    if ((srcBuffer->pixelData != NULL) && (dstBuffer->pixelData != NULL)) {
        int srcWidth = srcBuffer->width;
        int srcHeight = srcBuffer->height;
        PIXEL *srcPixelData = (PIXEL *) srcBuffer->pixelData;
        ALPHA *srcAlphaData = (ALPHA *) srcBuffer->alphaData;

        PIXEL *dstPixelData = (PIXEL *) dstBuffer->pixelData;
        ALPHA *dstAlphaData = (ALPHA *) dstBuffer->alphaData;

        blit(srcWidth, srcHeight, srcPixelData, srcAlphaData, x, y, width, height, dstPixelData, dstAlphaData, transform);
    }
