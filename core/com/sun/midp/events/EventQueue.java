...
/**
 * Contains methods for posting events and registering to receiving events from
 * both the Java layer and native layer and a dedicated thread for processing
 * events. Events are processed in the order they were posted.
 * <p>
 * Events can be posted from the native level or posted at the Java level. When
 * an event is posted at the Java level the event pre-process method of
 * associated event listener is called and at which time the event listener can
 * stop the event from being posted by returning false.
 * <p>
 * The run method of the EventQueue delivers events to event listeners and polls
 * the native event queue. When there are no events to deliver, the run method
 * waits after notifying the NativeEventMonitor to wake up and monitor the
 * native event queue.
 * </p>
 * <p>
 * When performing a non-blocking read of a native event, an a empty event
 * object will be passed to the native method and the native method will fill in
 * the empty event in one call, saving up to 3 native method calls per native
 * event over the old stream based implementation.
 * </p>
 */
public class EventQueue implements Runnable {
	...
	/**
	 * Constructs, assigns a native event queue handle, and starts the primary
	 * event queue for this isolate.
	 */
	static {
	...
	/**
	 * Event queue thread reference to synchronize on. The event queue is not
	 * used since it given out to callers. But is given to the native event
	 * queue monitor.
	 */
	...
	/**
	 * Holds the event listeners and dispatch data. Each listener will be placed
	 * at the index equal to the event type ID - 1 it registered for (zero is an
	 * invalid type). This array will start empty and grow if a listener is
	 * registered with an event type greater than table length - 1 table since 0
	 * is an invalid.
	 */
	...
	/**
	 * Returns true if the current thread is the event queue dispatch thread.
	 * 
	 ...
	 * 
	 * @param token
	 *            security token with the com.sun.midp permission "allowed"
	 * 
	...
	/**
	 * Get a reference to the event queue in a secure way. The calling suite
	 * must have the com.sun.midp permission "allowed".
	 * 
	 * @return MIDP event queue reference
	 */
	...
	EventQueue() {
		/*
		 * Currently we know of 33 events. The table will grow if needed. for
		 * testing we need to lower that without changing code.
		 */
		int tableSize = 33;
	...
	/**
	 * Clears native event queue for a given isolate - there could be some
	 * events from isolate's previous usage.
	 */
	private void resetNativeEventQueue() {
		// TODO
		if (Logging.TRACE_ENABLED)
			System.out.println("[DEBUG] EventQueue.resetNativeEventQueue(): not yet implemented");
	 ...
	 * assigned. If multiple Java event queue objects in the same isolate have
	 * native handles assigned, this causes serious problems.
	 * 
	 * @return Native event queue handle
	 */
	private int getNativeEventQueueHandle() {
		// TODO
		if (Logging.TRACE_ENABLED)
			System.out.println("[DEBUG] EventQueue.getNativeEventQueueHandle(): not yet implemented");
		return 0;
	}

	/**
	 * Native finalizer to reset the native peer event queue when the Isolate
	 * ends.
	 */
	 ...
	 * 
	 * @param eventType
	 *            eventType as an integer as listed in midpEvents.h
	 * @param listener
	 *            listener for the event type
	 * 
	 * @exception IllegalArgumentException
	 *                if the event type is not positive, or the listener is null
	 */
		...
		synchronized (eventQueueThread) {
			...
			if (Logging.REPORT_LEVEL <= Logging.WARNING) {
				if (dispatchTable[eventType - 1] != null) {
					Logging.report(Logging.WARNING, LogChannels.LC_CORE, "Warning overwriting event listener");
				}
			}
	...
	/**
	 * Post an event to the queue. The reference to the event will be put in the
	 * queue without copying, so the caller should not reuse events passed to
	 * this method. The implementation uses a dispatch table indexed by event
	 * type ID. So if there is no event listener registered for event either a
		...
		synchronized (eventQueueThread) {
			if (event.type < 1) {
				if (Logging.REPORT_LEVEL <= Logging.ERROR) {
					Logging.report(Logging.ERROR, LogChannels.LC_CORE, "Invalid event in the queue.");
				}
			...
			if (event.type > dispatchTable.length) {
				if (Logging.REPORT_LEVEL <= Logging.ERROR) {
					Logging.report(Logging.ERROR, LogChannels.LC_CORE, "Cannot dispatch: "
							+ "No listener for this event type: " + event.type);
				}
			...
			if (event.type != EventTypes.EVENT_QUEUE_SHUTDOWN) {
				...
				if (!dispatchData.listener.preprocess(event, dispatchData.waitingEvent)) {
			...
			if (Logging.REPORT_LEVEL <= Logging.INFORMATION) {
				numEvents++;
				if (numEvents > 30) {
					Logging.report(Logging.INFORMATION, LogChannels.LC_CORE, "*** queue overloaded, num events = "
							+ numEvents);
				}
			}
	 ...
	 * 
	 * @param eventType
	 *            event type ID of the to remove
	 * 
	 * @return the event removed or NULL if there is no event of the given type
	 *         in the queue
	 */
	...
	/**
	 * Shuts down the event queue after process any posted events. Will block
	 * until the queue is shut down.
	 * 
	 * @exception IllegalStateException
	 *                if called from the event thread
	 */
	...
	public void run() {
		...
		for (;;) {
			try {
				...
				// IMPL_NOTE
				if (Logging.REPORT_LEVEL <= Logging.INFORMATION) {
					Logging.report(Logging.INFORMATION, LogChannels.LC_CORE, "*** processing event " + event.type);
				}
				
				if (Logging.TRACE_ENABLED)
					System.out.println("[DEBUG] EventQueue.run(): event type: " + event.getType());
				...
				// IMPL_NOTE
				if (Logging.REPORT_LEVEL <= Logging.INFORMATION) {
					Logging.report(Logging.INFORMATION, LogChannels.LC_CORE, "*** processed event " + event.type);
				}
	 ...
	 * 
	 * @param t
	 *            the Throwable that caused the fatal error
	 */
	static void handleFatalError(Throwable t) {
		if (Logging.TRACE_ENABLED)
			System.out.println("[DEBUG] EventQueue.handleFatalError(): not yet implemented : " + t);
	}
}
